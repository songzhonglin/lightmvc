...
li {
 	list-style:none;width:106px; height:34px; 
 }
 ...
 #list li a{background-color:white}
</style>
<script>
		var path = '<%=basePath%>';
		var uid=${uid}
		...
		websocket.onclose = function(event) {
			...
			function sendMsg(){
				...
				if(v==""){
					...
					$("#content").append("<div class='tmsg'><label class='name'>"+fromName+"&nbsp;"+new Date().Format("yyyy-MM-dd hh:mm:ss")+"</label><div class='tmsg_text'>"+data.text+"</div></div>");
					scrollToBottom();
					$("#msg").val("");
				}
			}
			...
			function back(){
				debugger;
				//window.location.href=path +"/user/toOnline";
// 				 window.location.reload() 
			...
			$(function(){
				...
				$("#list li a").css("background-color","white");
...
<body>
	...
	<div id="contentList">
		<ul id="list">
		<c:forEach var="users" items="${onlineUser}" varStatus="state">
			<c:if test="${users.id != sessionScope.uid}">
				<li><a href="javascript:;" onclick="chooseValue(${users.id})">${users.name}</a></li>
			</c:if>			
		</c:forEach>
		</ul>
	</div>
